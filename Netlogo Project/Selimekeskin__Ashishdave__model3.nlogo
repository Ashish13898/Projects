...
to setup
  ...
  ask patches  [if random-float 100 < greendensity [ set pcolor green ]]

   ask patches [if random-float 100 < bluedensity [ set pcolor blue ]]

  ask patches  [if (distancexy ( xcor-Object1 * max-pxcor) ( ycor-Object1 * max-pxcor)) < Object1Size [ set pcolor sky ]]

  ask patches  [if (distancexy ( xcor-Object2 * max-pxcor) ( ycor-Object2 * max-pxcor)) < Object2Size [ set pcolor sky ]]
...
to go ;; turtle procedure
  if not flock-on? [ set color white set shape "default" p2 ]
 ifelse collect-on? [ collect ] [ set color white set shape "default" p2 ]

 if choose-settings? [ p9 ]

 ifelse trail-on? [ pen-down ][ pen-up ]

  p10
end

to collect  ;; turtle procedure
  P1 ;; search-for-chip
  P5 ;; find-new-pile
  P3 ;; put-down-chip
end

to p1  ;; turtle procedure -- "search-for-chip" by changing shape
    ifelse pcolor = green
  [ set pcolor black
    set shape "arrow"
    fd 20
...
to P2 ;; wiggle procedure
  forward 1
  right random 20
  left random 20
end

to p3 ;; turtle procedure -- "put-down-chip" by finds empty spot & drops chip
ifelse pcolor = black
[
  ifelse shape = "arrow"
   [ set pcolor green
...
to p4  ;; turtle procedure -- escape from piles
  rt random 360
  fd 20
  if pcolor != black
  [ p4 ]
...
to p5 ;; turtle procedure -- "find-new-pile" look for green patches
  if pcolor != green and pcolor != blue
  [
    p10
    p5
  ]
...
to p6 ;; flock
 ...
 ifelse any? flockmates [
    ...
    ifelse distance nearest-neighbor < nearview
    [ set color orange
 ][
   set color yellow
   face nearest-neighbor
    ...
    set color white
    let explore random exploration-option
    ifelse explore = 0 [
    rt search-angle
    ][
    lt search-angle
...
to p7 ;; find-flockmates
  set flockmates other turtles in-cone maxview vision-angle
 end

to p8 ;; find-nearest-neighbor
  set nearest-neighbor min-one-of flockmates [distance myself]
end

to p9
 if settings = 1 [set  vision-angle  0  set  perspace  0  set  nearview  0  set maxview 0 set search-angle 0 set exploration-option 2]
 if settings = 2 [set  vision-angle  0  set  perspace  0  set  nearview  0  set maxview 50 set search-angle 0 set exploration-option 2]
 if settings = 3 [set vision-angle 100 set perspace 0 set nearview 0 set maxview 50 set search-angle 0 set exploration-option 2]
 if settings = 4 [set vision-angle 100 set perspace 0 set nearview 15 set maxview 50 set search-angle 0 set exploration-option 2]
 if settings = 5 [set vision-angle 100 set perspace 0 set nearview 15 set maxview 50 set search-angle 45 set exploration-option 2]
 if settings = 6 [set vision-angle 100 set perspace 0 set nearview 15 set maxview 35 set search-angle 45 set exploration-option 2]
 if settings = 7 [set vision-angle 100 set perspace 0 set nearview 15 set maxview 35 set search-angle 45 set exploration-option 1]
 if settings = 8 [set vision-angle 100 set perspace 8 set nearview 15 set maxview 35 set search-angle 45 set exploration-option 2]
end

to p10 ;; to move
  ifelse flock-on? [ p6 ]  [ p2 ]
end
@#$#@#$#@
GRAPHICS-WINDOW
200
